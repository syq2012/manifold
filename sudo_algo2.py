...
def multi_weight(data, d, epoch, cod_dim, first_layer_dim, step_size):
...
#     cur_weight = np.array([0.1]*10 + [0.9/3]*3)
...
#         cur_valid = reweight_data(temp_valid, cur_weight)
        cur_data = encoder.get_dataset_from_list(cur_input, 0, 200)
        cur_valid = encoder.get_dataset_from_list(cur_input, 200, 250)
...
#         cur_weight = cur_weight * 0.5
        ...
        cur_test = encoder.get_dataset_from_list(cur_input, 300, 400)
...
#         print(np.sum(cur_weight))
    ...
    return cur_ae, cur_weight,1/total_round * average_weight, loss, test_error